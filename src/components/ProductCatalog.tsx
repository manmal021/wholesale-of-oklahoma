...
const CatalogCard: React.FC<CatalogCardProps> = ({ prod, onAdded }) => {
  ...
  return (
    <div className="group relative bg-white/80 backdrop-blur-xl rounded-[32px] p-6 border border-white hover:border-[#F97316]/40 transition-all duration-300 hover:shadow-xl flex flex-col justify-between">
      {/* Top badges */}
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {prod.subcategory && (
            <span className="bg-orange-50 border border-orange-200 text-orange-800 text-[9px] font-semibold px-2 py-0.5 rounded">
              {prod.subcategory}
            </span>
          )}
        </div>
        <span className="text-[11px] font-bold text-[#F97316] shrink-0 bg-orange-50/80 px-2 py-0.5 rounded-full border border-orange-100">
          Call for Price
        </span>
      </div>
      ...
      <div className="flex-1">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-2">
          {prod.puffs && (
            <span className="bg-[#0f172A] text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase">
              {prod.puffs}
            </span>
          )}
          {prod.size && (
            <span className="bg-[#F97316]/15 text-[#0f172A] text-[10px] font-bold px-2 py-0.5 rounded">
              {prod.size}
            </span>
          )}
          {prod.nicotine && (
        ...
        <div className="mt-4 space-y-1.5">
          {prod.features.slice(0, 3).map((feat, idx) => (
            <div key={idx} className="flex items-start gap-2 text-xs text-slate-600 py-0.5">
        ...
        {prod.flavours && prod.flavours.length > 0 && (
          <div className="mt-4">
            <h4 className="text-[11px] font-semibold text-[#0f172A]/70 mb-1.5">
            ...
            <div className="flex flex-wrap gap-1">
              {prod.flavours.slice(0, 4).map((flv, idx) => (
                <span
                  key={idx}
                  className="text-[10px] font-medium bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-200/60 text-slate-700"
                >
                  {flv}
                </span>
              ))}
      ...
      {/* Action Footer */}
      <div className="mt-6 pt-4 border-t border-slate-200/60 w-full space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-full px-2 py-1 shadow-2xs">
            <button
              type="button"
          ...
          <button
            ...
            className={`flex-1 py-2 text-white text-xs font-bold rounded-full transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm ${
              added ? 'bg-[#F97316]' : 'bg-[#0f172A] hover:bg-[#F97316]'
            }`}
          >
            {added ? (
              <>
        ...
        <button
          ...
          className="w-full text-center text-[10px] font-bold uppercase tracking-wider text-slate-500 hover:text-[#F97316] transition-colors py-0.5 flex items-center justify-center gap-1 cursor-pointer"
        >
...
export default function ProductCatalog() {
  ...
  return (
    <section id="how" className="py-16 md:py-24 bg-[#F8FAFC] text-[#0f172A] px-4 sm:px-6 md:px-10 border-t border-slate-200 relative">
      {/* Toast notification when item added from page */}
      {lastAddedNotice && (
        <div className="fixed top-24 right-6 z-40 bg-[#0f172A] text-white px-4 py-3 rounded-2xl shadow-2xl border border-white/10 flex items-center gap-3 animate-fade-in">
          <div className="w-8 h-8 rounded-full bg-[#F97316]/20 text-[#F97316] flex items-center justify-center shrink-0">
            <CheckCircle2 className="w-5 h-5" />
          </div>
          <div className="text-xs">
            <div className="font-bold">{lastAddedNotice}</div>
            <div className="text-[10px] text-slate-300">Updated in your Mango wholesale order</div>
          </div>
          <button
            onClick={handleOpenMangoCart}
            className="ml-2 bg-[#F97316] hover:bg-[#ea580c] text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1 transition-colors cursor-pointer"
          >
            <span>View Cart</span>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-10 gap-6">
          <div>
            ...
            <h2 className="text-3xl md:text-5xl font-semibold text-[#0f172A] tracking-tight">
              Best Sellers <span className="text-[#F97316]">in Stock Now</span>
            </h2>
          ...
          {cartCount > 0 && (
            <button
              onClick={handleOpenMangoCart}
              className="bg-[#0f172A] hover:bg-[#1e293b] text-white px-5 py-3 rounded-full shadow-lg flex items-center gap-2.5 transition-all self-start lg:self-auto cursor-pointer border border-white/10 group"
            >
              <ShoppingBag className="w-4 h-4 text-[#F97316] group-hover:scale-110 transition-transform" />
              <div className="text-left text-xs leading-tight">
                <span className="block font-bold">Wholesale Cart</span>
                <span className="text-[10px] text-[#F97316] font-semibold">{cartCount} items selected</span>
              </div>
              <ArrowRight className="w-4 h-4 ml-1 text-slate-400 group-hover:text-white transition-colors" />
            </button>
          )}
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-3xl p-4 border border-white shadow-sm mb-8 space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[#F8FAFC] border border-slate-200 rounded-full pl-10 pr-4 py-2 text-xs sm:text-sm text-[#0f172A] focus:outline-none focus:border-[#F97316] transition-colors"
            />
            {searchQuery && (
              <button
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-bold shrink-0 transition-all flex items-center gap-1.5 cursor-pointer select-none ${
                    isSelected
                      ...
                      : 'bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-[#0f172A]'
                  }`}
                >
                  <span>{cat.icon}</span>
                  <span>{cat.label}</span>
          ...
          {selectedCategory === 'Vape Mods & Kits' && (
            <div className="flex items-center gap-2 pt-2 border-t border-slate-100 overflow-x-auto pb-1">
              <span className="text-xs font-extrabold uppercase tracking-wider text-slate-400 shrink-0">
                Hardware Category:
              </span>
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-semibold shrink-0 transition-colors cursor-pointer ${
                      isSelected
                        ? 'bg-[#F97316] text-white font-bold shadow-2xs'
        ...
        <div className="flex items-center justify-between mb-6 text-xs text-slate-500 px-1">
          <div>
            Showing <strong className="text-[#0f172A]">{filteredProducts.length}</strong> products in{' '}
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-16 bg-white/50 rounded-3xl border border-white text-slate-400 space-y-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-[#0f172A] text-white text-xs font-bold rounded-full hover:bg-[#F97316] transition-colors cursor-pointer"
            >
              Reset Filters
            </button>
          </div>
