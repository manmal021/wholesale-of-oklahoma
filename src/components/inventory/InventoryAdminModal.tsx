...
export const InventoryAdminModal: React.FC<InventoryAdminModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      {/* Backdrop */}
      <div className="fixed inset-0 bg-[#1f2a1d]/75 backdrop-blur-sm" onClick={onClose} />

      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-white rounded-[32px] shadow-2xl border border-white/30 overflow-hidden z-10 animate-in fade-in zoom-in-95 duration-200 my-auto">
        {/* Header */}
        <div className="bg-[#1f2a1d] text-white p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#336443] flex items-center justify-center text-[#85AB8B]">
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="bg-[#f8faf8] p-5 rounded-2xl border border-neutral-200 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <button
                ...
                disabled={syncing}
                className="bg-[#1f2a1d] hover:bg-[#336443] text-white text-xs font-bold px-5 py-2.5 rounded-full flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm disabled:opacity-50"
              >
                <RefreshCw className={`w-3.5 h-3.5 text-[#85AB8B] ${syncing ? 'animate-spin' : ''}`} />
                {syncing ? 'Connecting to Zoho...' : 'Sync Now with Zoho'}
              </button>
            </div>

            {/* Sync Diagnostic Metrics */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-neutral-200/80 text-xs">
              <div className="bg-white p-3 rounded-xl border border-neutral-200">
                <span className="text-[10px] text-neutral-400 block font-semibold">Synced Items</span>
                <span className="text-base font-black text-[#1f2a1d]">
                  {syncStatus.total_items_synced}
                </span>
              </div>
              <div className="bg-white p-3 rounded-xl border border-neutral-200">
                <span className="text-[10px] text-neutral-400 block font-semibold">Sync Errors</span>
                <span className="text-base font-black text-[#1f2a1d]">
                  {syncStatus.items_with_errors}
                </span>
              </div>
              <div className="bg-white p-3 rounded-xl border border-neutral-200 col-span-2">
                <span className="text-[10px] text-neutral-400 block font-semibold">Last Successful Sync</span>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-[#1f2a1d] uppercase tracking-wider flex items-center gap-2">
              <Sliders className="w-4 h-4 text-[#85AB8B]" />
              Inventory Display Policy
            </h3>

            {/* 1. Exact Quantity vs Stock Status */}
            <div className="space-y-2 bg-neutral-50 p-4 rounded-2xl border border-neutral-200/70">
              <label className="text-xs font-bold text-[#1f2a1d] block">
                ...
                <button
                  ...
                  className={`p-3 rounded-xl text-left border text-xs font-semibold transition-all cursor-pointer ${
                    displayMode === 'status_only'
                      ? 'border-[#336443] bg-emerald-50/50 text-[#1f2a1d] ring-1 ring-[#336443]'
            ...
            <div className="flex items-center justify-between p-4 bg-neutral-50 rounded-2xl border border-neutral-200/70">
              <div>
                <span className="text-xs font-bold text-[#1f2a1d] block">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${
                  hideOutOfStock ? 'bg-[#336443]' : 'bg-neutral-300'
                }`}
              >
                <span
            ...
            <div className="p-4 bg-neutral-50 rounded-2xl border border-neutral-200/70 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#1f2a1d]">
                  Low Stock Threshold Warning
                </span>
                <span className="text-xs font-bold text-[#336443] bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
              ...
              <input
                ...
                className="w-full accent-[#336443] cursor-pointer"
              />
          ...
          <div className="bg-neutral-50 p-5 rounded-2xl border border-neutral-200/80 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-[#1f2a1d] uppercase tracking-wider flex items-center gap-1.5">
                <Server className="w-3.5 h-3.5 text-[#336443]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="space-y-1">
                ...
                <input
                  ...
                  id="zoho-client-id"
                  className="w-full bg-white border border-neutral-200 rounded-xl px-3 py-2 text-xs text-[#1f2a1d] focus:outline-none focus:border-[#336443]"
                />
              </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <span className="text-[10px] text-neutral-500">
                You can also enter these directly in your <code className="bg-neutral-200 px-1 py-0.5 rounded font-mono">.env</code> file.
              </span>
              <button
                type="button"
                onClick={async () => {
                ...
                className="bg-[#1f2a1d] hover:bg-[#336443] text-white text-[11px] font-bold px-4 py-2 rounded-full transition-colors cursor-pointer"
              >
          ...
          <div className="p-4 bg-neutral-900 text-white rounded-2xl space-y-2 text-xs">
            <div className="flex items-center gap-2 text-[#85AB8B] font-bold">
            ...
            <div className="bg-black/60 p-2.5 rounded-lg font-mono text-[11px] text-emerald-400 break-all select-all">
        ...
        {/* Footer Actions */}
        <div className="bg-neutral-100 p-4 px-6 flex items-center justify-between border-t border-neutral-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-[#336443] hover:bg-[#254c32] text-white text-xs font-bold px-5 py-2.5 rounded-full transition-colors cursor-pointer shadow-sm disabled:opacity-50"
            >
