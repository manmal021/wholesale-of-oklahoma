...
export default function GallerySlider() {
  ...
  return (
    <section id="gallery" className="py-16 md:py-24 bg-white text-slate-900 px-4 sm:px-6 md:px-10 overflow-hidden border-t border-slate-200">
      <div className="max-w-7xl mx-auto">

        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
          <div>
            <div className="inline-flex items-center gap-1.5 bg-orange-50 text-[#FF6B00] border border-orange-200 text-xs font-semibold px-3 py-1 rounded-full mb-3">
        ...
        >
              ...
              return (
                <div
                  ...
                  className={`relative overflow-hidden rounded-2xl transition-all duration-500 cursor-pointer flex-shrink-0 ${
                    isCenter
                      ? 'w-[55%] sm:w-[60%] h-full shadow-2xl border-2 border-[#FF6B00] z-10'
                      : 'w-[20%] sm:w-[18%] h-[75%] opacity-60 hover:opacity-90 shadow-md border border-slate-200'
                  }`}
                >
                  <img
                    src={images[imgIdx]}
                  ...
                  />
                  {isCenter && (
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-6">
                      <span className="inline-flex items-center gap-1.5 bg-white text-slate-900 text-xs font-bold px-4 py-2 rounded-full shadow-md">
                        <ZoomIn className="w-3.5 h-3.5 text-[#FF6B00]" />
                        View Full Size
                      </span>
                    </div>
                  )}
          ...
          <button
            onClick={prev}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white hover:bg-slate-50 text-slate-800 shadow-xl flex items-center justify-center transition-all z-20 cursor-pointer border border-slate-200"
          ...
          <button
            onClick={next}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white hover:bg-slate-50 text-slate-800 shadow-xl flex items-center justify-center transition-all z-20 cursor-pointer border border-slate-200"
